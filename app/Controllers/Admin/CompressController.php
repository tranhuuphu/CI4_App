...
{   
    ...
    {   
        ...
        for ($i=0; $i < $count; $i++) {
            ...
            if(!in_array($img[$i][0], $images_array_check, false)){
                ...
                try {
                    ...
                    $source->toFile($path6);
                    $data['image_size_compressed'] = filesize($path6);
        ...
        for ($i=0; $i < $count; $i++) {
            ...
            if(!in_array($img2[$i][0], $images_array_check, false)){
                ...
                try {
                    ...
                    $source->toFile($path);
                    $data3['image_size_compressed']   = filesize($path);
        ...
        return redirect()->to('admin/image')->with('success', "Nén thành công ảnh");
    }
    ...
    {   
            ...
            if($value['image_folder'] == 'tinymce'){
                if($value['image_TinyCME_status'] < 3){
                    ...
                    try {
                        ...
                        $source->toFile($path2);
                        $data2['image_size_compressed'] = filesize($path2);
                        $data2['image_folder'] = 'tinymce';
                        $imageModel->update($value['id'], $data2);
                    }
                    catch (\Tinify\Exception $e){

                        return redirect()->to('admin/image')->with('image', "Has Error");
                    }
                }
            }
        }
        ...
        return redirect()->to('admin/image')->with('image', "Nén thành công");
